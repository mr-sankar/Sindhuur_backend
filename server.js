...
const directories = ['Uploads/avatars', 'Uploads/documents', 'Uploads/stories'];
...
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    ...
    } else if (file.fieldname === 'storiesImage') {
...
app.get("/api/profiles", async (req, res) => {
  try {
    const users = await User.find({ "otp.verified": true }).select(
      "profileId personalInfo.name personalInfo.gender personalInfo.profileImage demographics.dateOfBirth professionalInfo.occupation location.city location.state professionalInfo.education demographics.community professionalInfo.income horoscope.generated photos subscription.current"
    );
